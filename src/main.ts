...
function createRequests() {
  ...
  try {
    ...
    if (!batchSheet) throw new Error('通知バッチシートが存在しません');
    ...
    for (let i = 1; i < vehicleData.length; i++) {
      ...
      const regCombined = buildRegistrationCombined(
        getCellValue(row, sourceHeader.regArea),
        getCellValue(row, sourceHeader.regClass),
        getCellValue(row, sourceHeader.regKana),
        getCellValue(row, sourceHeader.regNumber),
      );
...
function sendInitialEmails() {
  ...
  try {
    ...
    for (let i = 1; i < batchData.length; i++) {
      ...
      const subject = `【車両更新確認】半期一括 ${formatDateLabel(rangeStart || now, tz)}〜${formatDateLabel(
        rangeEnd || now,
        tz,
      )}`;
      const bodyText = [
      ...
      const bodyHtml = [
        '<p>本部長・副本部長 各位</p>',
        `<p>対象期間: ${escapeHtml(formatDateLabel(rangeStart || now, tz))}〜${escapeHtml(
          formatDateLabel(rangeEnd || now, tz),
        )}<br>回答期限: ${escapeHtml(formatDateLabel(deadline || now, tz))}</p>`,
...
function sendApprovalRequestEmails() {
  ...
  try {
    ...
    for (let i = 1; i < batchData.length; i++) {
      ...
      const rangeStart = parseDateValue(getCellRaw(row, batchHeader['対象開始日']));
      const rangeEnd = parseDateValue(getCellRaw(row, batchHeader['対象終了日']));
      const subject = `【車両更新確認】専務承認依頼 ${formatDateLabel(rangeStart || new Date(), tz)}〜${formatDateLabel(
        rangeEnd || new Date(),
        tz,
      )}`;
      const bodyText = [
...
function applyApprovalDecisions() {
  ...
  try {
    ...
    const batchSheet = ss.getSheetByName(SHEET_NAMES.NOTIFY_BATCHES);
    const vehicleSheet = ss.getSheetByName(VEHICLE_SHEET_NAME);
    if (!batchSheet || !vehicleSheet) throw new Error('必要シートが存在しません');
    ensureAppendColumns(vehicleSheet, VEHICLE_MASTER_EXTRA_HEADERS);
    const batchData = batchSheet.getDataRange().getValues();
    if (batchData.length <= 1) return;
    const batchHeader = getHeaderMap(batchData[0]);
    ...
    for (let i = 1; i < batchData.length; i++) {
      ...
      for (let r = 1; r < confirmData.length; r++) {
        ...
        if (decision !== SENMU_DECISION.APPROVE) continue;
...
function runTestSuite() {
  ...
  try {
    ...
    appendTestResult(
      '3月便: 期限(送付年3/31)',
      marchSchedule && marchSchedule.deadline.getTime() === expectedMarchDeadline.getTime() ? 'OK' : 'NG',
      marchSchedule ? formatDateLabel(marchSchedule.deadline, tz) : '期限なし',
    );
    ...
    appendTestResult(
      '9月便: 期限(同年9/30)',
      septemberSchedule && septemberSchedule.deadline.getTime() === expectedSeptemberDeadline.getTime() ? 'OK' : 'NG',
      septemberSchedule ? formatDateLabel(septemberSchedule.deadline, tz) : '期限なし',
    );
    ...
    appendTestResult(
      'リマインド: 期限10日前',
      reminderDate.getTime() === expectedReminderDate.getTime() ? 'OK' : 'NG',
      formatDateLabel(reminderDate, tz),
    );
    ...
    try {
      ...
      appendTestResult(
        '確認シート: 専務判断バリデーション',
        senmuValidation ? 'OK' : 'NG',
        senmuValidation ? '設定あり' : '設定なし',
      );
    } finally {
...
function parseMonthDayToDate(value: string, year: number, tz: string) {
...
function resolveSemiannualSchedule(now: Date, settings: ReturnType<typeof loadSettings>, tz: string) {
...
function applyConfirmSheetValidations(sheet: GoogleAppsScript.Spreadsheet.Sheet, rowCount: number) {
  const headerMap = getHeaderMap(sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0]);
  const setValidation = (headerName: string, options: string[]) => {
  ...
  const setCheckbox = (headerName: string) => {
    const col = headerMap[headerName];
    if (!col) return;
...
function protectSenmuColumns(sheet: GoogleAppsScript.Spreadsheet.Sheet, settings: ReturnType<typeof loadSettings>) {
  const headerMap = getHeaderMap(sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0]);
  const columns = ['専務判断', '専務コメント'];
  const editorSet: { [email: string]: boolean } = {};
  splitEmails(settings.senmuTo).forEach((email) => {
    editorSet[email] = true;
  });
  try {
    const effectiveUserEmail = Session.getEffectiveUser().getEmail();
    if (effectiveUserEmail) editorSet[effectiveUserEmail] = true;
  } catch (err) {
...
function formatConfirmVehicleLine(row: any[], headerMap: { [key: string]: number }, tz: string) {
...
function buildVehicleIndex(
  vehicleData: any[][],
  sourceHeader: ReturnType<typeof resolveSourceHeaders>,
  headerMap: { [key: string]: number },
) {
...
function resolveVehicleRowIndex(index: { [key: string]: number }, reg: string, chassis: string) {
  if (reg && chassis) {
    const key = normalizeVehicleKey(`${reg}__${chassis}`);
