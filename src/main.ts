      ...
      if (idxExisting.vehicleId) {
        for (let i = 1; i < existingData.length; i++) {
          ...
          if (sourceSheet && chassis) {
            ...
            if (!existingByVehicleId[key]) existingByVehicleId[key] = record;
          }
...
function applyMasterUpdates(batchId?: string) {
  ...
  try {
    ...
    for (let i = 1; i < confirmationData.length; i++) {
      ...
      if (isCheckedCell(getCellRaw(cRow, ch['マスター反映済み']))) continue;
      ...
      if (ch['マスター反映済み']) cRow[ch['マスター反映済み'] - 1] = true;
...
function sendReminderEmails() {
  ...
  try {
    ...
    if (settings.reminderMaxCount <= 0) return;
    ...
    for (let i = 1; i < requestData.length; i++) {
      ...
      const dept = getCellValue(row, reqHeader['管理部門']);
      const deptInfo = deptMaster[dept];
      if (!deptInfo || !deptInfo.active) continue;
      ...
      const status = getCellValue(row, reqHeader['ステータス']);
      if (status !== REQUEST_STATUS.SENT && status !== REQUEST_STATUS.RESPONDING) continue;

      const reminderCount = toNumber(getCellRaw(row, reqHeader['リマインド回数']), 0);
      if (reminderCount >= settings.reminderMaxCount) continue;

      const lastReminderAt = parseDateValue(getCellRaw(row, reqHeader['最終リマインド日時']));
      if (lastReminderAt && toDateOnly(lastReminderAt, tz).getTime() === today.getTime()) continue;
      ...
      if (today.getTime() < eligibleFrom.getTime()) continue;
...
function loadNotifiedRequestIds(type: string) {
  const ss = getSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_NAMES.NOTIFY_LOG);
  if (!sheet || sheet.getLastRow() <= 1) return {} as { [requestId: string]: boolean };
  const data = sheet.getDataRange().getValues();
  if (data.length <= 1) return {} as { [requestId: string]: boolean };
  const headerMap = getHeaderMap(data[0]);
  const result: { [requestId: string]: boolean } = {};
  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    if (getCellValue(row, headerMap['種別']) !== type) continue;
    const requestId = getCellValue(row, headerMap['requestId']);
    if (requestId) result[requestId] = true;
  }
  return result;
}
...
function doGet(e: GoogleAppsScript.Events.DoGet) {
  const message = 'このWeb回答ページは廃止されました。確認用シートでの入力運用へ移行済みです。';
...
function onRequestFormSubmit(e: GoogleAppsScript.Events.FormsOnFormSubmit) {
  // 半期一括フローではフォーム回答を使わないため、旧トリガーは無効化する。
  Logger.log('onRequestFormSubmit: 旧フォーム導線は廃止済みのため処理をスキップ');
}

function onApprovalFormSubmit(e: GoogleAppsScript.Events.FormsOnFormSubmit) {
  // 半期一括フローでは承認フォームを使わないため、旧トリガーは無効化する。
  Logger.log('onApprovalFormSubmit: 旧承認フォーム導線は廃止済みのため処理をスキップ');
}
...
function buildApprovalQueueSheetInternal() {
  ...
  const vehiclesByRequestId: { [requestId: string]: any[][] } = {};
  if (vehicleData.length > 1 && vehicleHeader['依頼ID']) {
    for (let i = 1; i < vehicleData.length; i++) {
      const row = vehicleData[i];
      const requestId = getCellValue(row, vehicleHeader['依頼ID']);
      if (!requestId) continue;
      if (!vehiclesByRequestId[requestId]) vehiclesByRequestId[requestId] = [];
...
function sendApprovalRequestEmails() {
  ...
  try {
    const ss = getSpreadsheet();
    const settings = loadSettings();
    if (!settings.approvalFlowEnabled) return;
    if (!settings.mailSendEnabled) {
    ...
    if (!requestSheet || !approvalSheet) return;
...
function applyApprovalDecisions() {
  ...
  try {
    ...
    const requestSheet = ss.getSheetByName(SHEET_NAMES.REQUESTS);
    const vehicleSheet = ss.getSheetByName(SHEET_NAMES.VEHICLE_VIEW);
    if (!approvalSheet || !requestSheet || !vehicleSheet) return;
    ...
    for (let i = 1; i < requestData.length; i++) {
      ...
      if (id) requestRowIndexById[id] = i;
    }
      ...
      if (input === APPROVAL_INPUT.APPROVE) {
        ...
        if (vehicleData.length > 1 && vehicleHeader['依頼ID'] && vehicleHeader['更新方針'] && vehicleHeader['最終決定']) {
          for (let v = 1; v < vehicleData.length; v++) {
            ...
            if (getCellValue(vr, vehicleHeader['依頼ID']) !== requestId) continue;
...
function formatVehicleLineForApproval(row: any[], headerMap: { [key: string]: number }, tz: string, rowIndex: number) {
...
function formatVehicleLineWithPolicy(row: any[], headerMap: { [key: string]: number }, tz: string, rowIndex: number) {
...
function reopenRequestForms(requestId: string) {
...
function ensureFormSubmitTrigger(form: GoogleAppsScript.Forms.Form) {
  const formId = form.getId();
  const triggers = ScriptApp.getProjectTriggers();
  const exists = triggers.some((trigger) => {
    if (trigger.getHandlerFunction() !== 'onRequestFormSubmit') return false;
...
function runTestSuite() {
  ...
  const batchRow = batchContext.row;
  const h = batchContext.headerMap;
  const reminderCell = getCellRaw(batchRow, h['リマインド送信日時']);
...
function loadVehicleViewContext() {
  const ss = getSpreadsheet();
  const vehicleSheet = ss.getSheetByName(SHEET_NAMES.VEHICLE_VIEW);
  if (!vehicleSheet) throw new Error('車両（統合ビュー）が存在しません。先に車両統合ビュー同期を実行してください。');
  ensureHeaders(vehicleSheet, 1, getSchemaHeaders(SHEET_NAMES.VEHICLE_VIEW));
  const vehicleData = vehicleSheet.getDataRange().getValues();
