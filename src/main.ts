...
function sendSenmuApprovalRequestIfReady(batchId?: string) {
  ...
  try {
    ...
    const confirmationHeader = confirmationData.length > 0 ? getHeaderMap(confirmationData[0]) : {};
    const counts = summarizeConfirmationSheetRows(confirmationData, confirmationHeader);
    if (counts.total <= 0) return resolvedBatchId;
    if (counts.unchecked > 0) {
...
function applySenmuDecisionFromSheet(batchId?: string) {
  ...
  try {
    ...
    appendNotificationLog(
      ...
      '',
      resolvedBatchId,
      `承認:${approved} 差戻し:${returned} 保留:${pending} 不正:${invalid}`,
    );
...
function sendHqReturnNotification(batchId?: string) {
  const lock = LockService.getDocumentLock();
  lock.waitLock(30000);
  try {
    const settings = loadSettings();
...
function sendMurataApprovalNotification(batchId?: string) {
  const lock = LockService.getDocumentLock();
  lock.waitLock(30000);
  try {
    const settings = loadSettings();
