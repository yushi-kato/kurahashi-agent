...
function syncVehicles() {
  ...
  try {
    ...
    const now = new Date();
    const seenVehicleId: { [vehicleId: string]: { sheet: string; rowIndex: number } } = {};
    const rowCount = Math.max(0, sourceData.length - 1);
    const vehicleIdValues: any[][] = [];
    const regCombinedValues: any[][] = [];
    ...
    if (rowCount > 0) {
      if (managedIndexes.vehicleId) sourceSheet.getRange(2, managedIndexes.vehicleId, rowCount, 1).setValues(vehicleIdValues);
      if (managedIndexes.regCombined) sourceSheet.getRange(2, managedIndexes.regCombined, rowCount, 1).setValues(regCombinedValues);
    }
...
function createBiannualBatch(options?: { suppressUi?: boolean }) {
  const lock = LockService.getDocumentLock();
  lock.waitLock(30000);
  try {
    const ss = getSpreadsheet();
    ...
    if (!notifyBatchSheet) throw new Error('通知バッチシートが存在しません');
...
function buildConfirmationSheetForLatestBatch() {
  ...
  try {
    ...
    if (!batchIdIndex) throw new Error('通知バッチの batchId 列が見つかりません');
...
function buildConfirmationSheet(batchId: string) {
  const ss = getSpreadsheet();
  const tz = ss.getSpreadsheetTimeZone();
  const notifyBatchSheet = ss.getSheetByName(SHEET_NAMES.NOTIFY_BATCH);
  if (!notifyBatchSheet) throw new Error('通知バッチシートが存在しません');
  ...
  const batchData = notifyBatchSheet.getDataRange().getValues();
  if (batchData.length <= 1) throw new Error('通知バッチにデータがありません');
  ...
  if (rows.length > 0) {
    ...
    if (answerCol) {
      const rule = SpreadsheetApp.newDataValidation().requireValueInList(ANSWER_OPTIONS as unknown as string[], true).build();
  ...
  const now = new Date();
  const setBatchCell = (headerName: string, value: any) => {
    const idx = batchHeader[headerName];
    if (idx) batchRow[idx - 1] = value;
  };
...
function sendHqInitialEmail(batchId?: string, options?: { suppressUi?: boolean }) {
  const lock = LockService.getDocumentLock();
  lock.waitLock(30000);
  try {
    const settings = loadSettings();
...
function sendHqReminderIfNeeded(batchId?: string) {
  const lock = LockService.getDocumentLock();
  lock.waitLock(30000);
  try {
    const settings = loadSettings();
    ...
    if (reminderSentAt) return resolvedBatchId;
    ...
    if (today.getTime() < reminderDate.getTime()) return resolvedBatchId;
...
function sendSenmuApprovalRequestIfReady(batchId?: string) {
  const lock = LockService.getDocumentLock();
  lock.waitLock(30000);
  try {
    const settings = loadSettings();
    ...
    if (sentAt) return resolvedBatchId;
    ...
    const counts = summarizeConfirmationSheetRows(confirmationData, confirmationHeader);
    if (hqGate.total <= 0) return resolvedBatchId;
...
function applySenmuDecisionFromSheet(batchId?: string) {
  const lock = LockService.getDocumentLock();
  lock.waitLock(30000);
  try {
    const snapshot = getBatchWorkflowSnapshot(batchId);
    ...
    if (!allDecided || !allChecked) {
      ...
      appendNotificationLog('専務判断反映', '', '', resolvedBatchId,
        `未達: 保留=${senmuGate.pending} 不正=${senmuGate.invalid} 未確認=${senmuGate.unchecked}`);
      return resolvedBatchId;
    }
    ...
    appendNotificationLog(
      ...
      '',
      resolvedBatchId,
      `承認:${senmuGate.approved} 差戻し:${senmuGate.returned} 保留:${senmuGate.pending} 不正:${senmuGate.invalid}`,
    );
...
function sendHqReturnNotification(batchId?: string, options?: { suppressUi?: boolean }) {
  const lock = LockService.getDocumentLock();
  lock.waitLock(30000);
  try {
    const settings = loadSettings();
    ...
    if (status !== BIANNUAL_BATCH_STATUS.SENMU_RETURNED) return resolvedBatchId;
    ...
    const returnedRows: { rowIndex: number; vehicleId: string; regNumber: string; comment: string }[] = [];
    ...
    if (senmuGate.pending > 0 || senmuGate.invalid > 0 || senmuGate.unchecked > 0) {
      appendNotificationLog(
        ...
        '',
        resolvedBatchId,
        `未入力(${senmuGate.pending})・不正(${senmuGate.invalid})・未確認(${senmuGate.unchecked})ありのため未送信`,
      );
      return resolvedBatchId;
    }

    if (returnedRows.length === 0) return resolvedBatchId;

    const batchLabel = getCellValue(row, headerMap['便区分']) || resolvedBatchId;
    const sheetUrl = buildSheetUrlWithGid(ss, confirmationSheet);
    const subject = `【差戻し】${batchLabel} 再確認のお願い`;
    const detailLines = returnedRows.map(
      (r) => `- ${r.regNumber}${r.comment ? '（' + r.comment + '）' : ''}`,
    );
    const body = [
      '本部長・副本部長 各位',
      '',
    ...
    for (const r of returnedRows) {
      ...
      if (ch['専務確認済み']) cRow[ch['専務確認済み'] - 1] = false;
    ...
    // バッチ行リセット
    const now = new Date();
    if (headerMap['専務依頼送信日時']) row[headerMap['専務依頼送信日時'] - 1] = '';
    if (headerMap['村田主任通知送信日時']) row[headerMap['村田主任通知送信日時'] - 1] = '';
    if (headerMap['リマインド送信日時']) row[headerMap['リマインド送信日時'] - 1] = '';
    row[headerMap['ステータス'] - 1] = BIANNUAL_BATCH_STATUS.INITIAL_SENT;
    row[headerMap['更新日時'] - 1] = now;
    notifyBatchSheet.getRange(1, 1, batchData.length, batchData[0].length).setValues(batchData);
...
function sendMurataApprovalNotification(batchId?: string, options?: { suppressUi?: boolean }) {
  const lock = LockService.getDocumentLock();
  lock.waitLock(30000);
  try {
    const settings = loadSettings();
    ...
    const status = getCellValue(row, headerMap['ステータス']);
    if (status !== BIANNUAL_BATCH_STATUS.SENMU_APPROVED) return resolvedBatchId;

    const murataSentAt = parseDateValue(getCellRaw(row, headerMap['村田主任通知送信日時']));
    if (murataSentAt) return resolvedBatchId;
...
type MasterAppliedEntry = {
  vehicleId: string;
  registration: string;
  policy: string;
  confirmationRowNumber: number;
  vehicleRowNumber: number;
  appliedDetails: string[];
};

function buildMasterAppliedDetailLine(entry: MasterAppliedEntry) {
...
function buildMasterAppliedMailBlock(entry: MasterAppliedEntry, index: number) {
  const lines = [
...
function notifyMurataMasterApplyCompleted(
  ...
  batchContext: ReturnType<typeof getNotifyBatchContext>,
  confirmationSheet: GoogleAppsScript.Spreadsheet.Sheet,
  appliedEntries: MasterAppliedEntry[],
  options?: { suppressUi?: boolean },
) {
...
function applyMasterUpdates(batchId?: string, options?: { suppressUi?: boolean }) {
  ...
  try {
    ...
    if (vehicleData.length <= 1) return resolvedBatchId;
    const vh = getHeaderMap(vehicleData[0]);
    const vehicleRowIndexById: { [vehicleId: string]: number } = {};
    ...
    const appliedEntries: MasterAppliedEntry[] = [];
...
function buildSheetUrlWithGid(ss: GoogleAppsScript.Spreadsheet.Spreadsheet, sheet: GoogleAppsScript.Spreadsheet.Sheet) {
...
function onEditSourceSync(e: GoogleAppsScript.Events.SheetsOnEdit) {
  if (!e || !e.range) return '';
  const range = e.range;
  const sheet = range.getSheet();
  if (!sheet || sheet.getName() !== PRIMARY_SOURCE_SHEET) return '';
  if (range.getRow() <= 1) return '';

  const settings = loadSettings();
  if (!tryReserveSourceSyncRun(settings.autoAdvanceMinIntervalSec)) return '';
  syncVehicles();
  return 'ok';
}

function onEditSettingsSync(e: GoogleAppsScript.Events.SheetsOnEdit) {
  if (!e || !e.range) return '';
  const sheet = e.range.getSheet();
  if (!sheet || sheet.getName() !== SHEET_NAMES.SETTINGS) return '';

  const data = sheet.getDataRange().getValues();
  const headerMap = getHeaderMap(data[0]);
  if (!headerMap['設定項目'] || !headerMap['値']) return '';
  const valCol = headerMap['値'];
  const startCol = e.range.getColumn();
  const endCol = startCol + e.range.getNumColumns() - 1;
  if (valCol < startCol || valCol > endCol) return '';
...
function advanceBiannualWorkflow(batchId?: string, reason?: string) {
  ...
  try {
    ...
    if (!phase) return targetBatchId;
    if (phase.phase === BATCH_PHASE.COMPLETED) return targetBatchId;
...
function seedSettings() {
  ...
  if (!existingKeys['自動進行_定期実行_間隔分']) {
    ...
    const intervalMinutes = normalizeAutoAdvanceIntervalMinutes(
      convertLegacyAutoAdvanceHours(legacyHours),
      Number(SETTINGS_DEFAULTS['自動進行_定期実行_間隔分']),
    );
...
function diagnoseStartLaunch(mode: (typeof START_LAUNCH_DIAG_MODE)[keyof typeof START_LAUNCH_DIAG_MODE]) {
  ...
  if (missingSettingLabels.length > 0) {
    blockingMessages.push(
      `設定シートの必須通知先が未入力です: ${missingSettingLabels.join(' / ')}`,
    );
  }

  if (schemaDriftMessages.length > 0) {
    warningMessages.push(
      `シート構造の不足を検知しました。syncSchema() で補完済みですが、念のため内容を確認してください。`,
    );
  }
  if (needsInputSummary.count > 0) {
    const reasonText =
      needsInputSummary.reasonCounts.length > 0
        ? `主な内容: ${needsInputSummary.reasonCounts.map((entry) => `${entry.reason} ${entry.count}件`).join(' / ')}`
        : '要入力シートに未整備の行があります。';
    if (mode === START_LAUNCH_DIAG_MODE.SCHEDULED) {
      blockingMessages.push(
        `要入力が ${needsInputSummary.count} 件あるため、自動開始を停止しました。${reasonText}`,
      );
    } else {
      warningMessages.push(
        `要入力は、台帳の未整備を知らせる一覧です。今回の開始は可能ですが、後続の確認や反映で詰まる原因になるため、早めの修正を推奨します。自動実行では停止対象です。${reasonText}`,
      );
    }
  }
  if (!settings.mailSendEnabled) {
    warningMessages.push(
      '通知_メール送信=FALSE です。メール送信を実施せず画面上の処理確認だけを行う設定なので、本番運用開始なら TRUE への切替が必要です。',
    );
  }
  if (existingBatch) {
    warningMessages.push(
      `同一便の既存バッチ ${existingBatch.batchId}（ステータス: ${existingBatch.status || '未設定'}）が見つかりました。続行すると新規起票ではなく既存バッチの再利用として動作します。`,
    );
  }
...
function summarizeNeedsInput() {
  ...
  if (!sheet || sheet.getLastRow() <= 1) {
    return {
      count: 0,
      reasonCounts: [] as { reason: string; count: number }[],
    };
  }
...
function findExistingBatchSummary(batchId: string) {
  const ss = getSpreadsheet();
...
function runBiannualScheduleCore(diagnosis?: ReturnType<typeof diagnoseStartLaunch>) {
...
function runBiannualScheduleWithSummary(diagnosis?: ReturnType<typeof diagnoseStartLaunch>) {
...
function notifyScheduledStartBlocked(diagnosis: ReturnType<typeof diagnoseStartLaunch>) {
  const settings = loadSettings();
...
function buildStartLaunchBody(diagnosis: ReturnType<typeof diagnoseStartLaunch>) {
  const lines: string[] = [];
  ...
  if (diagnosis.status === START_LAUNCH_STATUS.BLOCKED) {
    lines.push('');
    lines.push(
      'このまま開始すると、確認依頼が送れなかったり、対象抽出が不完全なまま進んだりして業務が中途半端に進むため、開始を止めています。',
    );
    lines.push('修正が必要な項目:');
...
function showStartLaunchConfirmDialog(diagnosis: ReturnType<typeof diagnoseStartLaunch>) {
...
function valuesFromSettingsSheet(data: any[][], headerMap: { [key: string]: number }, key: string) {
  const keyIndex = headerMap['設定項目'];
  const valueIndex = headerMap['値'];
  if (!keyIndex || !valueIndex) return '';
  for (let i = 1; i < data.length; i++) {
...
function cleanupTestData() {
  ...
  try {
    ...
    const testRegCombined: { [reg: string]: boolean } = {};
    ...
    if (sourceSheet) {
      ...
      if (data.length > 1) {
        ...
        const sourceIdx = resolveSourceHeaders(header) as any;
        const idx = {
        ...
        for (let i = 1; i < data.length; i++) {
          ...
          if (regCombined) testRegCombined[String(regCombined)] = true;
    ...
    if (needsInputSheet) {
      ...
      if (data.length > 1) {
        ...
        for (let i = 1; i < data.length; i++) {
          ...
          const isTest =
            (vehicleId && testVehicleIds[vehicleId]) ||
...
function cleanupUnusedSheets() {
  ...
  try {
    const ss = getSpreadsheet();
    const requiredSheetNames: { [name: string]: boolean } = {};
    SCHEMA_DEFS.forEach((def) => {
      requiredSheetNames[def.name] = true;
    });
    requiredSheetNames[PRIMARY_SOURCE_SHEET] = true;

    const activeConfirmationNames: { [name: string]: boolean } = {};
    const notifyBatchSheet = ss.getSheetByName(SHEET_NAMES.NOTIFY_BATCH);
    if (notifyBatchSheet && notifyBatchSheet.getLastRow() > 1) {
      const data = notifyBatchSheet.getDataRange().getValues();
      const headerMap = getHeaderMap(data[0]);
      ...
      if (confirmationIdx) {
        for (let i = 1; i < data.length; i++) {
          ...
          if (name) activeConfirmationNames[name] = true;
        }
      }
    }
...
function verifyAcceptanceCondition7LegacyNonReachable() {
  ...
  const wrapperExpectedSource: { [name: string]: string } = {
...
function installDailyTriggers() {
  ...
  const reminderBeforeDays = Math.max(0, toNumber(settings.reminderBeforeDays, 10));
  const dateKeys: { [key: string]: boolean } = {};
  const runDates: Date[] = [];
...
function findNotifyBatchRow(batchData: any[][], headerMap: { [key: string]: number }, batchId: string) {
  const batchIdIndex = headerMap['batchId'];
  if (!batchIdIndex) return null;
  for (let i = 1; i < batchData.length; i++) {
    const row = batchData[i];
    if (getCellValue(row, batchIdIndex) !== batchId) continue;
    return { row, rowIndex: i + 1 };
  }
  return null;
}

function getNotifyBatchContext(batchId?: string) {
  const ss = getSpreadsheet();
  const notifyBatchSheet = ss.getSheetByName(SHEET_NAMES.NOTIFY_BATCH);
  if (!notifyBatchSheet || notifyBatchSheet.getLastRow() <= 1) return null;
  ensureHeaders(notifyBatchSheet, 1, getSchemaHeaders(SHEET_NAMES.NOTIFY_BATCH));

  const batchData = notifyBatchSheet.getDataRange().getValues();
  if (batchData.length <= 1) return null;
  const headerMap = getHeaderMap(batchData[0]);
  const batchIdIndex = headerMap['batchId'];
  if (!batchIdIndex) return null;

  let rowInfo: { row: any[]; rowIndex: number } | null = null;
  if (batchId) {
...
function getBatchWorkflowSnapshot(batchId?: string) {
  const batchContext = getNotifyBatchContext(batchId);
...
function evaluateHqGate(data: any[][], headerMap: { [key: string]: number }) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (answer) result.hasAnyInput = true;
    if (!answer) result.pending += 1;
    if (!checked) result.unchecked += 1;
    if (answer && checked) result.completed += 1;
  }
...
function evaluateSenmuGate(data: any[][], headerMap: { [key: string]: number }) {
  const result = createEmptyGateEvaluation();
  if (!data || data.length <= 1) return result;
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (rawDecision) result.hasAnyInput = true;
    ...
    if (!decision) {
      ...
      result.completed += 1;
      if (decision === APPROVAL_INPUT.APPROVE) result.approved += 1;
      if (decision === APPROVAL_INPUT.RETURN) result.returned += 1;
    }
    if (!checked) result.unchecked += 1;
  }
...
function evaluateMurataGate(data: any[][], headerMap: { [key: string]: number }) {
  const result = createEmptyGateEvaluation();
  if (!data || data.length <= 1) return result;
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const decision = normalizeSenmuDecision(getCellValue(row, headerMap['専務判断']));
    if (decision !== APPROVAL_INPUT.APPROVE) continue;
    ...
    if (checked) result.hasAnyInput = true;
    if (!checked) result.unchecked += 1;
    if (!hasRequiredInputs) result.pending += 1;
    if (checked && hasRequiredInputs) result.completed += 1;
  }
...
function evaluateBatchPhase(batchId?: string) {
  const snapshot = getBatchWorkflowSnapshot(batchId);
  ...
  let phase: string = BATCH_PHASE.HQ_WAITING;
...
function shouldRunAutoSenmuDecision(batchId: string, phase?: { phase: string; senmuGate: any }) {
  const evaluated = phase || evaluateBatchPhase(batchId);
  if (!evaluated) return false;
  if (
    evaluated.phase !== BATCH_PHASE.SENMU_WAITING &&
...
function shouldRunAutoMasterUpdate(batchId: string, phase?: { phase: string; murataGate: any }) {
  const evaluated = phase || evaluateBatchPhase(batchId);
  if (!evaluated) return false;
...
function tryReserveSourceSyncRun(minIntervalSec: number) {
  return tryReserveRun(PROP_KEYS.SOURCE_SYNC_LAST_RUN_AT, minIntervalSec);
}

function tryReserveRun(propertyKey: string, minIntervalSec: number) {
...
function normalizeSenmuDecision(value: any) {
  ...
  if (text === APPROVAL_INPUT.RETURN) return APPROVAL_INPUT.RETURN;
  if (text === '承認済') return APPROVAL_INPUT.APPROVE;
  return '';
}

function isCheckedCell(value: any) {
...
function summarizeConfirmationSheetRows(data: any[][], headerMap: { [key: string]: number }) {
  const result = {
    total: 0,
    unchecked: 0,
    unanswered: 0,
...
function loadSourceVehicleContext() {
  const ss = getSpreadsheet();
  const vehicleSheet = ss.getSheetByName(PRIMARY_SOURCE_SHEET);
  if (!vehicleSheet) throw new Error('車両一覧が存在しません。先に車両一覧同期（要入力更新）を実行してください。');
  ensureHeaders(vehicleSheet, 1, getSchemaHeaders(PRIMARY_SOURCE_SHEET));
  const vehicleData = vehicleSheet.getDataRange().getValues();
  if (vehicleData.length === 0) {
    return { rows: [] as any[][], headerMap: {} as { [key: string]: number } };
  }
...
function buildUniqueSheetName(baseName: string) {
  const ss = getSpreadsheet();
...
function buildVehicleId(sourceSheet: string, regCombined: string, chassis: string, rowIndex: number) {
  ...
  if (reg && hasDigit) return `${sourceSheet}__${reg}`;
  if (ch) return `${sourceSheet}__${ch}`;
  if (reg) return `${sourceSheet}__${reg}__ROW${rowIndex}`;
...
function loadSettings() {
  ...
  return {
    ...
    autoAdvanceTimerIntervalMinutes: normalizeAutoAdvanceIntervalMinutes(
      values['自動進行_定期実行_間隔分'] || convertLegacyAutoAdvanceHours(values['自動進行_定期実行_間隔時間']),
      Number(SETTINGS_DEFAULTS['自動進行_定期実行_間隔分']),
    ),
...
function normalizeAutoAdvanceIntervalMinutes(value: any, fallback: number) {
  const desired = Math.max(1, Math.floor(toNumber(value, fallback)));
  if (AUTO_ADVANCE_INTERVAL_MINUTES.indexOf(desired) >= 0) return desired;
...
function writeArbitrarySheetData(sheet: GoogleAppsScript.Spreadsheet.Sheet, headers: string[], rows: any[][]) {
...
function toInlineJson(value: any) {
...
function protectViewSheet(sheetName: string) {
  const ss = getSpreadsheet();
  const sheet = ss.getSheetByName(sheetName);
  if (!sheet) return;
  ...
  try {
    ...
    try {
      ...
      if (editors && editors.length > 0) protection.removeEditors(editors);
    } catch (err) {
...
function protectSenmuColumns(sheetName: string) {
  ...
  try {
    ...
    protections.forEach((protection) => {
      if (protection.getDescription() !== desc) return;
      try {
        protection.remove();
      } catch (err) {
