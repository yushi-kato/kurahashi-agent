    ...
    SOURCE_SHEETS.forEach((sheetName) => {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        const regCombined = buildRegistrationCombined(
          getCellValue(row, headerIndexes.regArea),
          getCellValue(row, headerIndexes.regClass),
          getCellValue(row, headerIndexes.regKana),
          getCellValue(row, headerIndexes.regNumber)
        );
        const chassis = getCellValue(row, headerIndexes.chassis);
        const vehicleId = buildVehicleId(sheetName, regCombined, chassis, i + 1);
        const answer = answerMap[vehicleId];
  ...
  try {
    ...
    SOURCE_SHEETS.forEach((sheetName, index) => {
      ...
      if (!idx.regArea || !idx.regClass || !idx.regKana || !idx.regNumber || !idx.dept || !idx.contractEnd) {
        ...
        if (!idx.regArea) missing.push('地名');
        if (!idx.regClass) missing.push('分類番号');
        if (!idx.regKana) missing.push('かな');
        if (!idx.regNumber) missing.push('番号');
      ...
      for (let i = 1; i < data.length; i++) {
        const row = data[i];
        const regCombined = buildRegistrationCombined(
          getCellValue(row, idx.regArea),
          getCellValue(row, idx.regClass),
          getCellValue(row, idx.regKana),
          getCellValue(row, idx.regNumber)
        );
...
function findHeaderIndex(headerMap: { [key: string]: number }, names: string[]) {
