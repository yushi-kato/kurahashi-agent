...
3) 各部署はGoogleフォームで「更新方針」を回答する（更新/解約（入替）/解約（満了））  
4) 専務へ承認依頼フォームを送り、承認/差戻しを回収する  
5) 回答結果をスプレッドシート（台帳）へ反映する  
6) 部署別の集計（回答集計）を確認できる（必要なら管理者へサマリメール）
...
- `回答集計`
  - 部署別の回答数（更新/解約（入替）/解約（満了）/未回答）が見られます
- `承認待ち一覧`
  - 専務承認の進捗確認用です（承認フォーム回答の結果が反映されます）
