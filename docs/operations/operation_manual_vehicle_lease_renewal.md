...
年2回（**期間A便** / **期間B便**）、この流れで処理します。

| 便名 | 抽出条件 | 回答期限 |
| --- | --- | --- |
| **期間A便** | `期間Aの開始月` の1日より前に満了する車両 | 基準日の1か月後 |
| **期間B便** | `期間Bの開始月` の1日より前に満了する車両 | 基準日の1か月後 |
...
2. 「設定」シートで通知先と各期間の開始月を入力する（→ [設定項目の一覧](#設定タブの項目)）
...
| `期間Aの開始月` | 期間A便の基準月。1日を基準日に使う | `3` |
| `期間Bの開始月` | 期間B便の基準月。1日を基準日に使う | `9` |
...
`期間Aの開始月` / `期間Bの開始月` は、車両抽出の基準月です。入力した月の1日を基準日として使い、`契約満了日` がその日より前の車両を抽出します。回答期限は、その基準日の1か月後を自動で使います。
