...
function createBiannualBatch(options) {
    ...
    try {
        ...
        const targetVehicles = pickVehiclesBeforeContractEndDate(vehicleContext.rows, vehicleContext.headerMap, batchDef.targetEnd, tz);
        const now = new Date();
...
function buildStartLaunchErrorDiagnosis(mode, err) {
    return {
        ...
        warningMessages: [],
        missingSettingLabels: [],
        needsInputSummary: { count: 0, reasonCounts: [] },
        schemaDriftMessages: [],
        existingBatch: null,
...
function resolveBiannualBatchDefinition(referenceDate, tz, settings) {
    ...
    const upcoming = periods.find((period) => period.baseDate.getTime() > reference.getTime()) ||
...
function resolveConfiguredPeriods(year, settings, tz) {
...
function buildConfiguredPeriodDefinition(year, code, name, month, tz) {
...
function validateConfiguredPeriodMonths(periodAStartMonth, periodBStartMonth) {
...
function pickVehiclesBeforeContractEndDate(rows, headerMap, baseDate, tz) {
...
function loadSettings() {
    ...
    return {
        ...
        periodAStartMonth: resolveConfiguredPeriodMonthValue(values, '期間Aの開始月', ['半期基準日_3月', '半期送付日_3月'], Number(SETTINGS_DEFAULTS['期間Aの開始月'])),
        periodBStartMonth: resolveConfiguredPeriodMonthValue(values, '期間Bの開始月', ['半期基準日_9月', '半期送付日_9月'], Number(SETTINGS_DEFAULTS['期間Bの開始月'])),
...
function resolveConfiguredPeriodMonthValue(values, primaryKey, legacyKeys, fallback) {
...
function parseConfiguredMonthValue(rawValue, label) {
...
function validateConfiguredMonth(month, label) {
