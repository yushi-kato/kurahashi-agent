...
- GAS特有の制約・落とし穴・安定開発フローは `docs/guides/development_tips_for_gas.md` を参照する。
- スプレッドシートの「タブ/ヘッダ」スキーマをコードで管理するルールは `docs/guides/sheet_schema_management_rules.md` を参照する。
...
- `clasp run` を使ったテスト自動化（Workspace制限時の回避含む）は `docs/guides/test_automation_vehicle_lease_renewal.md` を参照する。
